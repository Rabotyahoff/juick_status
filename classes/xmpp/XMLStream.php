...
class XMPPHP_XMLStream {
	...
	public function connect($timeout = 30, $persistent = false, $sendinit = true) {
		...
		do {
			...
			try {
			  $this->log->log(' INFO. Start time='.time(),  XMPPHP_Log::LEVEL_ERROR);
			  $this->log->log(' INFO. timeout='.$timeout,  XMPPHP_Log::LEVEL_ERROR);

				$this->socket = stream_socket_client("$conntype://{$this->host}:{$this->port}", $errno, $errstr, $timeout, $conflag);
				//$this->socket = fsockopen("$conntype://{$this->host}", $this->port, $errno, $errstr, $timeout);
				//$this->socket = fopen("$conntype://{$this->host}:{$this->port}", 'r');

				$this->log->log(' INFO. Stop time='.time(),  XMPPHP_Log::LEVEL_ERROR);
			} catch (Exception $e) {
	...
	private function __process($maximum=5) {
		...
		/*Begin RA*/
		if ($this->socket==NULL && $this->sent_disconnect==true){
		  $this->disconnected = true;
		  return false;
		}
		/*End RA*/
